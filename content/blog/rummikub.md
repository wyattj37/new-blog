...
<a href="https://en.wikipedia.org/wiki/Rummikub" target="_blank">Rummikub</a> is quite a fun game; if you haven't played it, you should. If you have played, you might relate to the feeling of staring at the board and wondering if you truly don't have a move, or if a move is out there and you just can't find it. To fix this, I figured I'd build a tool to solve a given board state, and then I'll at least know after-the-fact what the best move would have been.

To make the solver, I formulate the game as a set partioning problem[^1] with a few adjustments. All the available tiles form one large set, and the rules for forming partitions are the rules which define runs and groups. The objective to be maximized in forming these partitions is the number of tiles used from the player's rack. The next section will go into describing this much more explicitly.
...
All this can be seen and used on my [GitHub](https://github.com/wyattj37).
